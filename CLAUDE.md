...
Fork replacement for the legacy "Running apps" screen (whose overflow entry is now hidden in `MainActivity` — superseded, not deleted). Entry point: the standalone gauge icon (`drawable/ic_monitor.xml`, custom `actionLayout` `view_toolbar_monitor.xml`) left of the filter icon on the main top bar; long-pressing the monitor's overflow opens the 白い熊 応用管理 UI page (same `attachOverflowLongPress` trick as MainActivity).
