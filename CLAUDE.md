...
- **Leak grouping** clusters ≥3 identical shell commands into one "comm ×N — LEAK" row (multi-pid `kill -9`); leaks are **pinned to the top** of the sort (they're tiny and would otherwise sink). Shared processes (one pid, many packages, e.g. `system_server`) dedupe into one row. Toolbar subtitle shows "N processes · M killable · L leaks".
- **Prefs / UI** (all in the `pref_monitor_*` "Process monitor" group on the UI page; live via per-bind reads + `onResume` separator reload): columns 1/2/3 (`MonitorPrefs`, top-bar grid toggle), app-icon size, row vertical padding, four row-state colours (`ColorPrefs.MONITOR_KILLABLE/LEAK/PROTECTED/USER_PROTECTED`), and the separator grid (`MonitorSeparatorPrefs` + `ColorPrefs.MONITOR_SEPARATOR_H/V`, drawn by `MonitorSeparatorDecoration` — a copy of `MainSeparatorDecoration` that also draws a top line on the first row).
