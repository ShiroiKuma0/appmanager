...
3. `Fix cleanupDocs CC regression: use File.deleteDir() GDK extension` — corrective hotfix on top of commit 2, replacing the broken call to upstream's script-level `def deleteDir(File)` helper with Groovy's GDK `File.deleteDir()`. One file. Documented below as **commit 3**. *Logically belongs in commit 2; kept separate for now to avoid history-rewrite costs. Squash into commit 2 on the next history-cleanup pass.*
4. `Add custom build numbering and profile-filter negation` — versioning system (gradle.properties additions, app/build.gradle defaultConfig override, `tools/bump-build.sh` script) and the profile-filter-negate UI + viewmodel logic. Ten files. Documented below as **commit 4**. *Logically belongs in commit 1; kept separate for the same reason. Squash into commit 1 on the next history-cleanup pass.*
5. `Fix mergeAssets ordering: explicit dependsOn on :server:create*ServerJars` — second corrective hotfix on commit 2. Adds an explicit `mergeAssetsProvider.dependsOn ":server:create<Variant>ServerJars"` in app/build.gradle so the server-runner JARs (`am.jar`, `main.jar`) are reliably packaged into the APK under CC + parallel scheduling. One file. Documented below as **commit 5**. *Logically belongs in commit 2; squash there on the next history-cleanup pass alongside commit 3.*
6. `Add main list freeze indicator with auto-refresh after freeze/unfreeze` — snowflake badge + italic label + dimmed icon for frozen apps (any of: PM-disabled, suspended, hidden) in the main package list, plus live-PM enrichment so the indicator survives DB cache staleness and delayed broadcasts, plus an explicit `BroadcastUtils.sendPackageAltered` call after freeze/unfreeze so the list refreshes automatically. Six files modified + one new vector drawable. Documented below as **commit 6**. *Logically belongs in commit 1; squash there on the next history-cleanup pass alongside commit 4.*
7. `Add 'Skip freeze method dialog' preference under Settings -> Rules` — new `SwitchPreferenceCompat` plumbed end-to-end (AppPref enum, Prefs.Blocking accessors, XML preference, strings, AppInfoFragment branch). When enabled, the App-info Freeze action skips the method-picker dialog and freezes directly with the resolved method. Bulk freeze dialogs are intentionally unaffected. Five files. Documented below as **commit 7**. *Logically belongs in commit 1; squash there alongside commits 4 and 6 on the next history-cleanup pass.*

Each commit's edits are small and surgical, designed for trivial conflict resolution on rebase. The target structure is two commits (one customization layer, one CC layer); the seven-commit state is a working intermediate that defers the history rewrite. When adding NEW customizations going forward: prefer appending another small commit if it's logically distinct, or amend into the most natural existing commit if the change is corrective. Don't chase the "exactly two commits" ideal at the cost of risky autosquash rebases — overlapping diffs between commits 1 and 2 make `--fixup` + `--autosquash` brittle for this project specifically.
...
One file, corrective hotfix on top of commit 2. Already described in detail in the "Commit 2 — section 3. docs/build.gradle" subsection above (the "Plus a non-obvious CC bug" paragraph). When folding into commit 2 during a future history cleanup, this commit's diff goes into the same docs/build.gradle hunks as commit 2's cleanupDocs refactor.

## Commit 4 — `Add custom build numbering and profile-filter negation`

Ten files. Two logically distinct features bundled into one commit because they shipped together; on the next history cleanup, fold this commit's content into commit 1 alongside the side-by-side-install changes.
...
If MuntashirAkon eventually adds new `BuildConfig.DEBUG`-gated features we want exposed, fold them into commit 1 (or commit 4 alongside the existing customizations — either works since both layers are us). New CC fixes go into commit 2 or commit 3. Avoid stacking many new commits indefinitely — a history-cleanup pass to collapse 3 into 2 and 4 into 1 is appropriate when convenient, but not urgent.
...
- **Squash commits 3 and 5 into commit 2, commit 4 into commit 1.** Five-commit `origin/custom` structure is a working intermediate; target is two clean commits (customization layer + CC layer). Skip this until a calm session where briefly breaking the build is cheap. The autosquash-with-fixup approach is brittle for this project — commits 1 and 2 have overlapping diff context on `gradle.properties` and `app/build.gradle`, so a fixup generated against the combined state can't replay cleanly onto commit 1 alone. Likely path: interactive rebase with `edit` actions on commits 1 and 2, then re-stage the right files at each stop.
