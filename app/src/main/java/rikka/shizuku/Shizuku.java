...
public class Shizuku {
    ...
    // Fork: true when the server accepted only the v11 attach (code 14) and refused the v13 one
    // (code 17). See the note in onBinderReceived: such a server routes code 14 to attachApplication
    // in its legacy switch, which means requestPermission() — also code 14 in the v13 AIDL — is
    // swallowed there and can NEVER reach showPermissionConfirmation. Callers use this to skip a
    // prompt that will never appear and send 白い熊 to the manager's own authorisation UI instead.
    private static volatile boolean legacyAttach = false;
    ...
    public static void onBinderReceived(@Nullable IBinder newBinder, String packageName) {
        ...
        if (newBinder == null) {
            ...
            // Fork (白い熊, measured on-device 2026-07-30, cost build +45): each attach attempt is
            // guarded SEPARATELY, because a THROWN v13 attach must still fall through to v11.
            //
            // LANDMINE: upstream chains the two with `&&`, which only falls back when v13 returns
            // false — but a server that does not dispatch code 17 to attachApplication answers with
            // an exception, not false. 白い熊 雫's Service.onTransact decides "legacy vs new client"
            // by comparing the interface token against BOTH
            // "moe.shizuku.server.IShizukuService" and ShizukuApiConstants.BINDER_DESCRIPTOR —
            // which are the SAME string, so `isLegacy` always wins and the `else` branch holding
            // "code 17 = attachApplication v13+" is dead code. Code 17 therefore falls past the
            // legacy switch and lands on a method that calls requireClient(), which throws
            // IllegalStateException("Not an attached client"). Upstream's `&&` short-circuit never
            // reaches v11, so the client ends up holding a binder it is not attached to: every
            // later checkSelfPermission()/requestPermission() throws the same, no prompt is ever
            // shown, and the mode reports "Shizuku did not authorise" with nothing on screen.
            //
            // Attaching on code 14 with the v11 marshalling — (IBinder, String) — is exactly what
            // that server's legacy branch handles, so the fallback below succeeds where v13 cannot.
            // Keep BOTH attempts: a server that does honour code 17 is served by the first, and this
    ...
    /**
     * Fork: whether the attach succeeded only on the legacy transaction (code 14).
     * <p>
     * On such a server the v13 permission API is unreachable — the legacy dispatch consumes code 14
     * as {@code attachApplication}, so {@link #requestPermission(int)} is answered with
     * {@code writeNoException()} and nothing else happens. Authorisation has to come from the
     * manager's own UI instead.
     */
    public static boolean isLegacyAttach() {
        return legacyAttach;
    }
