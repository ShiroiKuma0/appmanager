...
public class Ops {
    ...
    public static int init(@NonNull Context context, boolean force) {
        ...
        try {
            return initInternal(context, force);
        } finally {
    ...
    @NoOps // Although we've used Ops checks, its overall usage does not affect anything
    @Status
    private static int initInternal(@NonNull Context context, boolean force) {
        String mode = getMode();
        sDirectRoot = hasRoot();
    ...
    private static void autoDetectRootSystemOrAdbAndPersist(@NonNull Context context) {
        ...
        if (LocalServices.alive()) {
            setMode(sIsShizuku ? MODE_SHIZUKU : MODE_ADB_OVER_TCP);
            int uid = Users.getSelfOrRemoteUid();
            if (uid == ROOT_UID) {
                sIsSystem = sIsAdb = false;
                sIsRoot = true;
        ...
        setModeFromAutoDetection(getWorkingUid() != Process.myUid() ? MODE_ADB_OVER_TCP : MODE_NO_ROOT,
                shizukuPresent);
    }
