...
public final class PackageInstallerCompat {
        ...
        public void onReceive(Context context, @NonNull Intent intent) {
            ...
            switch (intent.getAction()) {
                ...
                case ACTION_INSTALL_COMPLETED:
                    ...
                    // Fork: see statusToString() -- a failure must never be reported with no reason.
                    if (mStatusMessage == null && mFinalStatus != STATUS_SUCCESS) {
                        mStatusMessage = statusToString(mFinalStatus);
                    }
    ...
    private boolean commit(int userId) {
        ...
        if (intentReceiver == null) {
            ...
            // Fork: THIS is the branch that swallowed the reason. STATUS_PENDING_USER_ACTION is
            // handled in PackageInstallerBroadcastReceiver, i.e. only on the *unprivileged* path;
            // here it arrives as "not STATUS_SUCCESS" and is reported as a nameless failure. It is a
            // prompt rather than a fault -- the session is still live and the platform is asking for
            // a confirmation this path has no way to show -- so say which of the two it was.
            if (mFinalStatus == PackageInstaller.STATUS_PENDING_USER_ACTION) {
                Log.w(TAG, "Commit: the platform wants an explicit user confirmation for this session.");
            }
            if (mStatusMessage == null && mFinalStatus != STATUS_SUCCESS) {
                mStatusMessage = statusToString(mFinalStatus);
            }
