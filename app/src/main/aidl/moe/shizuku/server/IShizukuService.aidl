// Fork: trimmed copy of the Shizuku client AIDL, vendored from the sister repo
// shiroikuma-shizuku (api/aidl). Only the transactions that exist in BOTH stock Shizuku and
// 白い熊 雫 (shiroikuma.shizuku) are declared here — the Plus-only additions (ids 107..121,
// returning af.shizuku.server.* interfaces) are deliberately left out so this one client works
// against either server.
//
// The transaction ids are the wire format and are pinned explicitly upstream; NEVER renumber or
// reorder them, and never insert a method without its own explicit id. The interface descriptor
// must stay "moe.shizuku.server.IShizukuService" for the same reason.
